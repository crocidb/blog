...
description: "I spent a whole month researching FreeBSD's kernel's virtual memory system to understand why tools like htop, btop and fastfetch report different information about RAM usage. I ended up submitting patches to all of these projects."
date: 2026-07-02
thumb: images/Pastedimage20260618233614.png
tags:
 - freebsd
 - operating-systems
 - investigation
...
Now we get to the interesting part. Both of these tools, and many others such as `htop`, try to report the memory usage so the user (or sysadmin) can have an idea of what's going on with their systems. For that, they all have to pick a _heuristic_; effectively decide what they'll call **used memory**. And the whole difference comes from the fact that they have different heuristics.
...
But then now I had a plan on how to proceed with it in `btop`: basically subtract the _variable_ size of the ARC cache from the wired memory. Also, just like `htop`, consider `vfs.bufspace` as reclaimable cache. Here's the [PR](https://github.com/aristocratos/btop/pull/1728). As the time of writing, it hasn't been reviewed or approved yet.
...
<video controls width="100%">
  <source src="images/testing-btop-cache.webm" type="video/webm">
</video>
